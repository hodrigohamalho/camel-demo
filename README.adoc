...
For it, we need a Kafka cluster running, so deploy https://strimz.io[AMQ Streams] in your Openshift with the name `my-cluster`.

Uncomment the first line `camel.component.kafka.brokers` from the `application.properties` file, and recreate as a secret in Openshift. 
